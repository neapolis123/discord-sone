...
async def get_filling(ticker_dict,session,notified_or_discarded,days_limit=number_of_days_for_fillings):  # we hit the SEC API to get the fillings from 30 days that has EFFECT or S-1
    ...
    if response.status == 403: # sometimes the server just throttles us
        ...
        current_errors = errors.get(formated_error_timestamp_tunis_time,0) # then save it in a set to be consulted on demand 
        print(f'current errors count for timestamp {formated_error_timestamp_tunis_time} are {current_errors}')
    ...
    try: # frequently the server returns code 200 but it actually timed without the correct payload, for this we try and catch this here, we do it again at line 230 for the IPO checks
        hits = int(api_response['hits']['total']['value'])
    except Exception:
        t = await response.text() 
        code = response.status 
        if code == 500:
           print(f'SEC Server Internal Error at link {url}')       
        else :      
           print(f'SEC Server problem with response code {code} and {t} at link : {url}')
        return 
       ...
       return # returns NONE here that gets filtered on the function that called it
        ...
        try:       
           ...
           if int(api_response['hits']['total']['value']):  #its an IPO, discard
              ...
              t = await response.text()  
              if code == 500:
                   print(f'SEC Server Internal Error at link {url_CERT}')    
              else:
                   print(f'SEC Server problem with response code {code} and {t} at link : {url_CERT} ')  
              return
        ...
        async with aiohttp.ClientSession(headers=headers) as s: # means we got a newer filling for a notified or a discarded ticker or simply first time check for something that has non IPO fillings, we check if they are good or not inside 
            for form in forms: # if forms are returned we check if they match F-1/X or S-1/X,
                    ...
                    filling_text = filling_text.replace('&nbsp;',' ').replace('\n',' ') # to catch example SUBJECT&nbsp;TO&nbsp;COMPLETION ( view-source:https://www.sec.gov/Archives/edgar/data/1874252/000121390024106670/ea0223498-f1a1_mainz.htm) and to eliminate line breaks : view-source:https://www.sec.gov/Archives/edgar/data/1874252/000121390024106670/ea0223498-f1a1_mainz.htm and view-source:https://www.sec.gov/Archives/edgar/data/2028336/000121390025100507/ea0261579-s1a2_newera.htm
                    eliminating_text = ['will not receive any proceeds from any sale','We will not receive any proceeds from any such sales','will not receive any of the proceeds from sales','will not receive any proceeds from the sale', 'will not receive any proceeds from the resale','will not receive any of the proceeds from the sale','will not receive any of the proceeds from the resale',' will not receive any of the proceeds from these sales','will not receive proceeds from the sale'] # view-source:https://www.sec.gov/Archives/edgar/data/109657/000149315225019296/forms-1.htm, edge canse , a good S1 with a filter word KTTA : https://www.sec.gov/Archives/edgar/data/1841330/000121390025115595/ea0267537-s1a1_pasithea.htm
                    ...
                    if all(el not in filling_text for el in eliminating_text) : #longer version :'will not receive any proceeds' not in filling_text and 'will not receive any of the proceeds' not in filling_text: # this checks if the S-1/F-1 filling is NOT a shareholders selling filling by checking for the eliminating text
                        ...
                        if 'SUBSCRIPTION RIGHTS TO PURCHASE' in filling_text.upper() :# sometimes we check for a rights offerings if it's not a sellings shareholder or an annex
                            print(f'Rights offering found on {ticker_dict["ticker"]} with url {filling_link}, discarded') # for debugging purposes 
                        else: 
