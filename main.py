       ...
       return # returns NONE here that gets filtered on the function that called it
        ...
        async with aiohttp.ClientSession(headers=headers) as s: # means we got a newer filling for a notified or a discarded ticker or simply first time check for something that has non IPO fillings, we check if they are good or not inside 
            for form in forms: # if forms are returns we check if they match F-1/X or S-1/X,
                    ...
                    if 'This page is temporarily unavailable' in filling_link: # checks if the SEC server is down, happenes from time time, in this case we basically reject the ticker so we don't notify every ticker that has S-1
                        print(f"SEC site is down when trying to retreive ticker {ticker_dict['ticker']} with url: {url}")
                        return
