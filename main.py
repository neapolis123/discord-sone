       ...
       return # returns NONE here that gets filtered on the function that called it
        ...
        async with aiohttp.ClientSession(headers=headers) as s: # means we got a newer filling for a notified or a discarded ticker or simply first time check for something that has non IPO fillings, we check if they are good or not inside 
            for form in forms: # if forms are returned we check if they match F-1/X or S-1/X,
                    ...
                    eliminating_text = ['will not receive any proceeds from','We will not receive any proceeds from any such sales','will not receive any of the proceeds from sales','will not receive any proceeds from the sale', 'will not receive any proceeds from the resale','will not receive any of the proceeds from the sale','will not receive any of the proceeds from the resale',' will not receive any of the proceeds from these sales','will not receive proceeds from the sale'] # view-source:https://www.sec.gov/Archives/edgar/data/109657/000149315225019296/forms-1.htm
